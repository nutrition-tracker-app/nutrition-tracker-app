import { 
  ...
  export const createMeal = async (mealData) => {
    try {
      ...
      if (mealToAdd.allNutrients && mealToAdd.allNutrients.length > 0) {
        console.log(`Saving meal with ${mealToAdd.allNutrients.length} detailed nutrients`);
      } else {
  ...
  export const trackSleep = async (userId, bedtime, wakeup, quality = 5) => {
    try {
      ...
      const bedtimeDate = typeof bedtime === 'string' ? new Date(bedtime) : bedtime;
      ...
      console.log(`Sleep calculation: Start=${bedtimeDate}, End=${wakeupDate}, Duration=${durationMinutes} minutes`);
  ...
  export const trackExercise = async (userId, category, duration, intensity = 'moderate', calories = 0) => {
    try {
      if (!userId) {
        throw new Error('User ID is required');
      }
